...
```
t2s-ui  ->  t2s-api  ->  Postgres / Redis / S3
                    |
                    v
                 SQS queue
                    |
                    v
              t2s-worker -> LLM gateway / eval artifacts
```

## What this demonstrates

- **Release ownership**: `t2s-api`, `t2s-worker`, and `t2s-ui` are deployed as one Argo CD application with independent rollouts and shared environment promotion.
...
Use this directory when asked, "How would you run T2S on Falcon/Kubernetes?"
